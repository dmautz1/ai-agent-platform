...
async def get_agents(user: Dict[str, Any] = Depends(get_optional_user)):
    ...
    with perf_logger.time_operation("get_agents", user_id=user["id"] if user else None):
...
async def get_agent_info(
    ...
    with perf_logger.time_operation("get_agent_info", user_id=user["id"] if user else None, agent_name=agent_name):
...
async def get_agent_health(
    ...
    with perf_logger.time_operation("get_agent_health", user_id=user["id"] if user else None, agent_name=agent_name):
...
async def get_all_agent_configs(user: Dict[str, Any] = Depends(get_current_user)):
    ...
    with perf_logger.time_operation("get_all_agent_configs", user_id=user["id"]):
...
async def get_available_profiles(user: Dict[str, Any] = Depends(get_current_user)):
    ...
    with perf_logger.time_operation("get_available_profiles", user_id=user["id"]):
