"""
Unit tests for Grok Service and Configuration

Tests the Grok service functionality including authentication setup,
...
# Test imports
from config.grok import (
    GrokConfig,
    get_grok_config,
    validate_grok_environment
)
from services.grok_service import GrokService, get_grok_service


class TestGrokConfig(unittest.TestCase):
    """Test cases for GrokConfig class functionality"""
    ...
    def setUp(self):
        ...
        self.original_env = {
            ...
            'GROK_DEFAULT_MODEL': os.getenv('GROK_DEFAULT_MODEL')
        }
    ...
    def test_grok_config_initialization(self):
        """Test Grok configuration initialization"""
        os.environ['GROK_API_KEY'] = 'xai-test-key-123'
        
        config = GrokConfig()
        
        self.assertEqual(config.api_key, 'xai-test-key-123')
        self.assertEqual(config.default_model, 'grok-beta')

    def test_grok_config_with_custom_model(self):
        """Test Grok configuration with custom model"""
        os.environ['GROK_API_KEY'] = 'xai-test-key'
        os.environ['GROK_DEFAULT_MODEL'] = 'grok-2'
        
        config = GrokConfig()
        
        self.assertEqual(config.default_model, 'grok-2')
    ...
    def test_get_available_models(self):
        ...
        expected_models = ["grok-beta", "grok-vision-beta"]
    ...
    def test_get_model_info(self):
        ...
        config = GrokConfig()
        info = config.get_model_info('grok-beta')
    ...
    def test_get_model_info_default(self):
        ...
        self.assertIn('description', info)
        # Should return info for default model (grok-beta)
        self.assertIn('Grok', info['description'])
    ...
    def test_test_connection_success(self):
        ...
        self.assertEqual(result['model'], 'grok-beta')
        self.assertIn('xai-', result['api_key_prefix'])

    def test_test_connection_missing_key(self):
        """Test connection test with missing API key"""
        config = GrokConfig.__new__(GrokConfig)
        config.api_key = None
        config.default_model = 'grok-beta'
        
        result = config.test_connection()
        
        self.assertEqual(result['status'], 'error')
        self.assertIn('No Grok API key', result['error'])
    ...
    def test_test_connection_invalid_format(self):
        ...
        self.assertEqual(result['status'], 'error')
        self.assertIn('Invalid Grok API key format', result['error'])


class TestGrokModule(unittest.TestCase):
    """Test cases for Grok module functions"""
    
    @patch('config.grok.GrokConfig')
    def test_get_grok_config(self, mock_grok_config_class):
        """Test get_grok_config function"""
        mock_config = Mock()
        mock_grok_config_class.return_value = mock_config
        
        # Clear any existing global config
        import config.grok
        config.grok.grok_config = None
        
        result = get_grok_config()
        
        self.assertEqual(result, mock_config)
        mock_grok_config_class.assert_called_once()

    @patch('config.grok.get_grok_config')
    def test_validate_grok_environment_success(self, mock_get_config):
        """Test successful environment validation"""
        mock_config = Mock()
        mock_config.test_connection.return_value = {'status': 'success'}
        mock_get_config.return_value = mock_config
        
        result = validate_grok_environment()
        
        self.assertTrue(result['valid'])
        self.assertEqual(len(result['errors']), 0)

    @patch('config.grok.get_grok_config')
    def test_validate_grok_environment_connection_failure(self, mock_get_config):
    ...
    @patch('config.grok.get_grok_config')
    def test_validate_grok_environment_missing_config(self, mock_get_config):
...
class TestGrokService:
    """Test cases for GrokService functionality"""
    
    def setup_method(self):
        """Set up test fixtures"""
        with patch('services.grok_service.get_grok_config') as mock_config, \
             patch('services.grok_service.openai.AsyncOpenAI') as mock_client:
            
            mock_config.return_value = Mock()
            mock_config.return_value.api_key = 'xai-test-key'
            mock_config.return_value.base_url = 'https://api.x.ai/v1'
    ...
    def test_service_initialization(self):
        ...
        assert hasattr(self.service, '_grok_config')
    ...
    async def test_query_success(self):
        ...
        self.service._grok_config.get_available_models.return_value = ['grok-beta']
        self.service._grok_config.default_model = 'grok-beta'
    ...
    async def test_query_with_all_parameters(self):
        ...
        self.service._client.chat.completions.create = AsyncMock(return_value=mock_response)
        self.service._grok_config.get_available_models.return_value = ['grok-2']
        self.service._grok_config.default_model = 'grok-beta'
    ...
    def test_get_info(self):
        ...
        self.service._grok_config.default_model = 'grok-beta'
        self.service._grok_config.get_available_models.return_value = ['grok-beta', 'grok-2']
        
        info = self.service.get_info()
        
        assert info['service'] == 'Grok'
        assert info['default_model'] == 'grok-beta'
        assert 'available_models' in info

    def test_test_connection(self):
        """Test connection testing"""
        self.service._grok_config.test_connection.return_value = {
            'status': 'success',
            'service': 'Grok'
        }
...
class TestGrokServiceSingleton:
    """Test Grok service singleton functionality"""
    
    def test_get_grok_service_singleton(self):
        """Test that get_grok_service returns singleton"""
        with patch('services.grok_service.get_grok_config'), \
             patch('services.grok_service.openai.AsyncOpenAI'):
